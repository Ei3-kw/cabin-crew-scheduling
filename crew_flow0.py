...
class CrewFlowNetwork:
    ...
    def build_model(self):
        ...
        for base in self.airports:
            ...
            # DEBUG: report whether non-home supply nodes have outbound variables
            if self.position_supply and base in self.position_supply:
                for ap, cnt in self.position_supply[base].items():
                    if ap == base or cnt == 0:
                        continue
                    depot_node = Node(airport=ap, time=DEPOT_TIME_START)
                    has_vars = bool(outs.get(depot_node))
                    print(f"  DEBUG [build_model] base={base} crew_at={ap} "
                          f"count={cnt} has_outbound_vars={has_vars}")
    ...
    def _decompose_routes(
        ...
        The key correctness invariant: flow injected at depot node D can only
        be consumed by paths that originate at D.  The MIP enforces this via
        per-depot supply constraints, so Gurobi's solution guarantees exactly
        `count` outbound flow units at each (base, depot_airport) node.  The
        decomposer must preserve this partition — a single shared residual lets
        a path starting at SFO drain flow that belongs to BOS-origin paths.

        Fix: decompose each (base, depot_airport) group in isolation.  Build a
        separate residual for each source depot, trace exactly `count` paths
        through it, then release the residual remainder back to a shared pool
        for the home-depot group (which absorbs any flow that originates at home
        and passes through non-home airports mid-route).

        Parameters
        ----------
        crew_positions : {crew_id: airport}
            Where each crew member is at the start of this window.
            None → first window, everyone starts at home.

        Returns
        -------
        (routes, next_crew_positions)
        """
        # ── Build global residual keyed (base, arc) ───────────────────────────
        full_residual: dict[tuple[str, Arc], int] = {}
        for (base, arc), var in self.var_work.items():
            val = round(var.X)
            if val > 0:
                full_residual[(base, arc)] = full_residual.get((base, arc), 0) + val
        for (base, arc), var in self.var_dh.items():
            val = round(var.X)
            if val > 0:
                full_residual[(base, arc)] = full_residual.get((base, arc), 0) + val
        ...
        for base in self.airports:
            ...
            # Full arc pool for this base (shared; we'll carve source-partitions
            # out of it and put remainder back into the home group).
            base_residual: dict[Arc, int] = {}
            for (b, arc), cap in full_residual.items():
                if b == base and cap > 0:
                    base_residual[arc] = base_residual.get(arc, 0) + cap

            # ── Determine depot groups ────────────────────────────────────────
            # position_supply[base] = {airport: count} from the previous window.
            # For each non-home depot airport, we know exactly how many paths
            # must originate there.  The home depot gets whatever is left.
            ps = (self.position_supply or {}).get(base, {}) if self.position_supply else {}
            # depot_groups: list of (depot_node, crew_ids_for_this_depot)
            # processed non-home depots first, home depot last.
            depot_groups: list[tuple[Node, list[int]]] = []

            # Map crew to their carry-over airport.
            crew_airport: dict[int, str] = {
                cid: next_crew_positions.get(cid, base)
                for cid in base_crew_ids
            }
            # Group crew by their starting airport.
            by_airport: dict[str, list[int]] = defaultdict(list)
            for cid, ap in crew_airport.items():
                by_airport[ap].append(cid)

            # Non-home depots first (most constrained — exact flow count).
            for ap in sorted(by_airport):
                if ap == base:
                    continue
                depot_groups.append((Node(airport=ap, time=DEPOT_TIME_START),
                                     by_airport[ap]))
            # Home depot last.
            depot_groups.append((home, by_airport.get(base, [])))

            # ── Process each depot group in isolation ─────────────────────────
            for depot_node, group_crew_ids in depot_groups:
                ap = depot_node.airport
                n_paths = len(group_crew_ids)
                if n_paths == 0:
                    continue

                # Extract only arcs reachable from this depot — do a forward
                # BFS over the residual graph to find all reachable arcs, then
                # work with only those arcs.  This prevents paths from bleeding
                # into arc-space that belongs to a different source depot.
                reachable_arcs = self._reachable_arcs(
                    depot_node, base_residual
                )
                # Carve out this depot's sub-residual.
                depot_residual: dict[Arc, int] = {
                    arc: base_residual[arc]
                    for arc in reachable_arcs
                    if arc in base_residual
                }

                flow_out = sum(cap for arc, cap in depot_residual.items()
                               if arc.start == depot_node)
                if ap != base:
                    print(f"  DEBUG [decompose] base={base} crew_at={ap} "
                          f"expected={n_paths} flow_departing={flow_out}")
                    if flow_out < n_paths:
                        print(f"  WARNING [decompose] base={base} depot={ap}: "
                              f"flow {flow_out} < crew {n_paths} — "
                              f"{n_paths - flow_out} crew will sit idle at this depot.")
                else:
                    # Home depot: flow may be less than crew count (idle crew = valid MIP outcome)
                    idle = max(0, n_paths - flow_out)
                    if idle > 0:
                        print(f"  INFO [decompose] base={base}: "
                              f"{flow_out} paths for {n_paths} crew "
                              f"({idle} crew sit idle — valid MIP outcome)")

                for crew_id in group_crew_ids:
                    prior_hb   = self._hb_states.get(crew_id, {})
                    prior_airport = crew_airport[crew_id]

                    carry_hw = HOME_BREAK  # default: gate open (away from home or first window)
                    if prior_airport == base:
                        prior_home_arr = prior_hb.get('home_arr', None)
                        if prior_home_arr is not None:
                            carry_hw = min(HOME_BREAK,
                                           max(0, win_start_abs - prior_home_arr))
                        # else: fully rested (HOME_BREAK)

                    path_arcs = self._trace_path(
                        depot_node, horizon, depot_residual,
                    ...
                    if not path_arcs:
                        # Check whether flow actually remained — if yes, BFS
                        # failed to find a path despite available capacity, which
                        # is a genuine decomposition bug.  If no, the crew simply
                        # has no work this window (valid MIP idle decision).
                        flow_available = sum(
                            cap for arc, cap in depot_residual.items()
                            if arc.start == depot_node
                        )
                        if flow_available > 0:
                            print(
                                f"  ERROR [decompose] crew {crew_id} (base {base}, "
                                f"depot {ap}): BFS failed despite {flow_available} "
                                f"outbound flow units remaining. Check home-break gate."
                            )
                        # else: flow exhausted — crew sits idle, valid MIP outcome.
                        continue

                    # Commit: remove consumed arcs from depot_residual AND
                    # base_residual so subsequent depots see updated capacity.
                    for arc in path_arcs:
                        depot_residual[arc] = depot_residual.get(arc, 0) - 1
                        if depot_residual[arc] <= 0:
                            depot_residual.pop(arc, None)
                        base_residual[arc] = base_residual.get(arc, 0) - 1
                        if base_residual[arc] <= 0:
                            base_residual.pop(arc, None)
                    ...
                    if legs:
                        prior = self._hb_states.get(crew_id, {})
                        ...
                        self._hb_states[crew_id] = new_state

                        if viols:
                            n_hb = getattr(self, '_hb_violation_count', 0)
                            self._hb_violation_count = n_hb + len(viols)
                            if n_hb == 0:
    ...
    def _reachable_arcs(
        ...
        """
        Forward BFS over the residual graph from `source`.
        Returns the set of all arcs with residual capacity reachable from source.
        Used to carve out a depot-local sub-residual for source-partitioned
        flow decomposition.
        """
        ...
        while queue:
            node = queue.popleft()
            for arc, cap in [(a, c) for a, c in residual.items()
                             if a.start == node and c > 0]:
                found.add(arc)
                if arc.end not in visited:
                    visited.add(arc.end)
                    queue.append(arc.end)
        return found
    ...
    def _trace_path(self, source, sink, residual, exit_cutoff=None, base=None, carry_home_wait=HOME_BREAK):
        from collections import deque

        type_priority = {'flight': 0, 'deadhead': 1, 'wait': 2}

        # KEY CHANGE: visited state is (node, hw_bucket) not just node.
        # We discretize hw into two buckets: < HOME_BREAK (gated) vs >= HOME_BREAK (open).
        # This avoids exponential blowup while correctly separating gated/open states.
        HW_OPEN  = HOME_BREAK   # sentinel for "gate open"

        parent: dict[tuple[Node, bool], tuple[Node, bool, Arc] | None] = {}
        # State: (node, gate_open) where gate_open = hw >= HOME_BREAK
        start_state = (source, carry_home_wait >= HOME_BREAK)
        parent[start_state] = None
        home_wait_val: dict[tuple[Node, bool], int] = {start_state: carry_home_wait}
        ...
        while queue:
            state = queue.popleft()
            node, gate_open = state
            hw_here = home_wait_val[state]
            ...
            if node == sink:
                ...
                while parent[cur_state] is not None:
                    prev_state, arc = parent[cur_state]
                    path.append(arc)
            ...
            candidates = [
                ...
                if residual.get(a, 0) > 0
            ]

            # Home-break gate
            if base is not None and node.airport == base and not gate_open:
                candidates = [
                    a for a in candidates
                    if a.arc_type == 'wait' or a.end.airport == base
                ]
            ...
            for arc in candidates:
                ...
                next_gate_open = hw_next >= HOME_BREAK
                next_state = (arc.end, next_gate_open)

                if next_state not in parent:
                    parent[next_state] = (state, arc)
                    home_wait_val[next_state] = hw_next
                    queue.append(next_state)

        # Reconstruct from best terminal
        if best_terminal_state is not None:
            path = []
            cur_state = best_terminal_state
            while parent[cur_state] is not None:
                prev_state, arc = parent[cur_state]
                path.append(arc)
                cur_state = prev_state
