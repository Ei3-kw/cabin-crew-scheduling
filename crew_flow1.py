...
Method:
  ...
  - Variables: integer FLOW per arc (not per crew member) for waits/deadheads;
               per-CREW binary variables for flight arcs to give a guaranteed
               per-crew coverage constraint.
      f_work[arc]       = # crew working this arc (aggregate base-level)
      f_crew[cid, arc]  ∈ {0,1} = 1 iff crew member cid works this flight arc
  ...
  - Per-crew coverage: sum_cid f_crew[cid, flight_arc] >= flight.min_crew
  - Linking: sum_{cid of base} f_crew[cid, arc] == f_work[base, arc]
...
Key advantages over fully individual-variable formulation:
  - Wait/deadhead variables: O(|arcs|) instead of O(|crew| × |arcs|)
  - Flight variables: O(|crew| × |flight_arcs|) — per-crew guarantee, but flight
    arcs are a small fraction of all arcs so variable count stays manageable
  - Coverage is a true per-crew MIP guarantee (not reconstructed post-solve)
  - Flow balance constraints: O(|nodes|) (shared for wait/dh arcs)
  - Crew identity comes directly from var_work_crew in Stage 2 — no ambiguity
...
class CrewFlowNetwork:
    ...
    with per-crew coverage guarantee on flight arcs.
    ...
    Variables:
      f_work[base, arc]  ∈ ℤ≥0   aggregate working crew from base on arc
      f_crew[cid, arc]   ∈ {0,1} 1 iff crew member cid works this flight arc
      f_dh[base, arc]    ∈ ℤ≥0   deadheading crew from base on arc
      f_wait[base, arc]  ∈ ℤ≥0   waiting crew on this arc
      f_wait_home[cid, a]∈ ℝ≥0   per-crew home wait (for mandatory home-break)
    ...
    Linking (per-crew ↔ aggregate):
      ∑_{cid of b} f_crew[cid, arc]  ==  f_work[b, arc]   ∀ b, arc

    Per-crew coverage guarantee:
      ∑_{cid} f_crew[cid, flight_arc] + slack[flt] >= flt.min_crew

    After solving, Stage 2 flow decomposition reads f_crew directly to assign
    named crew IDs to routes — no ambiguity, no post-hoc heuristic.
    ...
    def __init__(
        ...
        self.var_wait:      dict[tuple[str, Arc], gp.Var] = {}  # away wait arcs only
        # Per-crew FLIGHT work variables: (crew_id, arc) -> Var  ∈ {0,1}.
        # Created for every (crew_id, flight_arc) pair where the crew's home base
        # has a var_work variable for that arc.  Gives a true per-crew coverage
        # guarantee in the MIP rather than relying on post-hoc flow decomposition.
        # Linked to var_work via:  sum_{cid of base} var_work_crew[(cid,arc)]
        #                           == var_work[(base, arc)]
        self.var_work_crew: dict[tuple[int, Arc], gp.Var] = {}
        # Per-crew home wait variables: (crew_id, arc) -> Var.
        # Only created for wait arcs at the crew's home airport.  Splitting home
        # waits by crew is the minimum individuation needed to write a true
        # per-crew mandatory home-break constraint without fully individuating
        # every variable in the model.
        self.var_wait_home: dict[tuple[int, Arc], gp.Var] = {}
    ...
    def __getstate__(self):
        ...
        for key in ('model', 'var_work', 'var_work_crew', 'var_dh', 'var_wait',
                    'var_wait_home', 'slack_var', 'flow_constrs', 'coverage_constrs'):
            state.pop(key, None)
        return state

    def __setstate__(self, state):
        import copy
        for key in ('var_work', 'var_work_crew', 'var_dh', 'var_wait', 'var_wait_home',
    ...
    def build_model(self):
        """
        Build the MIP with integer flow variables per arc for waits and deadheads,
        plus binary per-crew variables for flight arcs (per-crew coverage guarantee).

        Uses batch variable creation and sparse constraint building for speed.

        Variable types
        --------------
        var_work[(base, arc)]       : int >= 0   aggregate working crew from base on arc
        var_work_crew[(cid, arc)]   : binary     = 1 iff crew member cid works this arc
        var_dh[(base, arc)]         : int >= 0   deadheading crew from base on arc
        var_wait[(base, arc)]       : int >= 0   crew waiting on away arc
        var_wait_home[(cid, arc)]   : cont [0,∞) per-crew home wait (home-break enforcement)

        Key constraints added
        ---------------------
        link_{base}_{arc_id}  : sum_{cid of base} var_work_crew[(cid,arc)]
                                 == var_work[(base,arc)]
        cov_{flight_id}       : sum_{cid} var_work_crew[(cid,arc)] + slack >= min_crew
        """
        import time as _t
        self._ensure_attrs()
        t0 = _t.time()
        ...
        dh_objs:   list[float] = []
        wait_keys: list[tuple[str, Arc]] = []      # away wait arcs only
        wait_objs: list[float] = []
        wait_home_keys: list[tuple[int, Arc]] = []  # (crew_id, arc) for home waits
        wait_home_objs: list[float] = []

        # Identify home-wait arcs per base: wait arcs whose start airport is
        # the base itself.  These will be individualised per crew member.
        home_wait_arcs_by_base: dict[str, list[Arc]] = defaultdict(list)
        for arc in wait_arcs:
            if arc.start.airport in self.airports:
                home_wait_arcs_by_base[arc.start.airport].append(arc)

        # Map crew_id -> base for fast lookup
        crew_base_map: dict[int, str] = {c.id: c.base for c in self.crew}
        # Crew ids grouped by base
        crew_ids_by_base: dict[str, list[int]] = defaultdict(list)
        for c in self.crew:
            crew_ids_by_base[c.base].append(c.id)
        ...
        for base in self.airports:
            ...
            for arc in wait_arcs:
                if arc.id not in reach:
                    continue
                if arc.start.airport == base:
                    # Home wait arc — will be individualised per crew member below.
                    pass
                else:
                    wait_keys.append((base, arc))
                    wait_objs.append(arc.cost)

        # Per-crew home wait variables: one per (crew_id, home_wait_arc).
        # Cost is 0 (home waits are free), same as the aggregate version.
        for base, home_arcs in home_wait_arcs_by_base.items():
            reach = reachable[base]
            for arc in home_arcs:
                if arc.id not in reach:
                    continue
                for cid in crew_ids_by_base[base]:
                    wait_home_keys.append((cid, arc))
                    wait_home_objs.append(arc.cost)  # 0 for home arcs
        ...
        if wait_home_keys:
            _whvars = self.model.addVars(
                len(wait_home_keys), lb=0, obj=wait_home_objs, vtype=GRB.CONTINUOUS,
                name=[f"wth_{cid}_{a.id}" for cid, a in wait_home_keys])
            self.var_wait_home = {k: _whvars[i] for i, k in enumerate(wait_home_keys)}

        # ── Per-crew flight work variables (per-crew coverage guarantee) ──────
        # For every (base, flight_arc) in var_work, create one binary variable
        # per crew member of that base:  var_work_crew[(cid, arc)] ∈ {0,1}.
        # Cost = 0 here; the flight cost stays on var_work to avoid double-counting.
        # These vars are linked to var_work via:
        #   sum_{cid of base} var_work_crew[(cid,arc)] == var_work[(base,arc)]
        # and coverage is expressed directly over var_work_crew so the MIP
        # guarantees that exactly min_crew DISTINCT individuals cover each flight.
        work_crew_keys: list[tuple[int, Arc]] = []
        for base in self.airports:
            for arc in flight_arcs:
                if (base, arc) not in self.var_work:
                    continue
                for cid in crew_ids_by_base[base]:
                    work_crew_keys.append((cid, arc))

        if work_crew_keys:
            _wc_vars = self.model.addVars(
                len(work_crew_keys), lb=0, ub=1, obj=0.0, vtype=GRB.CONTINUOUS,
                name=[f"fc_{cid}_{a.id}" for cid, a in work_crew_keys])
            self.var_work_crew = {k: _wc_vars[i] for i, k in enumerate(work_crew_keys)}
        ...
        self.model.update()
        total_vars = (len(self.var_work) + len(self.var_work_crew) + len(self.var_dh) +
                      len(self.var_wait) + len(self.var_wait_home) + len(self.slack_var))
        print(f"  Variables: {total_vars:,}  "
              f"(incl. {len(self.var_work_crew):,} per-crew flight, "
              f"{len(self.var_wait_home):,} per-crew home-wait)  ({_t.time()-t0:.1f}s)")

        # ── Sparse flow-balance constraints ───────────────────────────────────
        # Away wait arcs and flight arcs contribute to the aggregate (base-level)
        # flow balance.  Home wait arcs contribute at two levels:
        #   - Aggregate (base): sum over all crew of var_wait_home[cid, arc]
        #   - Per-crew: each individual crew member's home-node balance
        #
        # Per-crew home balance is enforced at every home node n for each
        # crew member c:
        #   Σ_{a out of n, home-wait} var_wait_home[c,a]
        #   + Σ_{a out of n, flight/dh} var_work/dh[base,a]  (shared — best effort)
        #   - Σ_{a into n, ...} ... == inj[c,n]
        #
        # Because flight/dh variables are still shared across the base, we cannot
        # write a fully individualised balance at non-home nodes without
        # exploding variable count.  Instead we keep the aggregate balance for
        # all nodes and ADD per-crew home-wait coupling constraints that force
        # each individual to consume exactly their share of home wait flow.
        ...
        # Per-crew home-node terms: {crew_id: {node: [vars_out]}} and in
        crew_home_out: dict[int, dict[Node, list]] = defaultdict(lambda: defaultdict(list))
        crew_home_in:  dict[int, dict[Node, list]] = defaultdict(lambda: defaultdict(list))
        ...
        for (base, arc), var in self.var_wait.items():
            out_terms[base][arc.start].append(var)
            in_terms[base][arc.end].append(var)
        # Home-wait vars: contribute to both aggregate base balance AND
        # the per-crew home-node balance used for the individual home-break constraint.
        for (cid, arc), var in self.var_wait_home.items():
            base = crew_base_map[cid]
            out_terms[base][arc.start].append(var)
            in_terms[base][arc.end].append(var)
            crew_home_out[cid][arc.start].append(var)
            crew_home_in[cid][arc.end].append(var)
        ...
        # ── Linking constraints: var_work_crew ↔ var_work ─────────────────────
        # For each (base, flight_arc) pair in var_work, the sum of per-crew
        # binary variables for crew of that base must equal the aggregate flow:
        #   sum_{cid of base} var_work_crew[(cid, arc)] == var_work[(base, arc)]
        # This ties the per-crew coverage variables tightly to the flow network
        # so the solver cannot assign crew to flights they don't actually traverse.
        n_link = 0
        for base in self.airports:
            cids = crew_ids_by_base[base]
            for arc in flight_arcs:
                if (base, arc) not in self.var_work:
                    continue
                crew_vars = [
                    self.var_work_crew[(cid, arc)]
                    for cid in cids
                    if (cid, arc) in self.var_work_crew
                ]
                if not crew_vars:
                    continue
                self.model.addConstr(
                    gp.quicksum(crew_vars) == self.var_work[(base, arc)],
                    name=f"link_{base}_{arc.id}"
                )
                n_link += 1

        self.model.update()
        print(f"  Linking constraints (var_work_crew==var_work): {n_link:,}  ({_t.time()-t0:.1f}s)")

        # ── Per-crew home-node flow balance ───────────────────────────────────
        # Each crew member c at base b must have balanced flow at every home
        # node, considering only the home-wait arcs individualised to them.
        # This couples the per-crew vars to the overall flow topology.
        #
        # For each crew c at base b and each home node n:
        #   Σ_{a∈home_wait_out(n)} var_wait_home[c,a]
        #   - Σ_{a∈home_wait_in(n)} var_wait_home[c,a]  == inj_c(n)
        #
        # where inj_c(n) = 1 if n is this crew's starting depot node, -1 at
        # horizon (last window), 0 everywhere else.
        #
        # NOTE: this balance is only over home-wait arcs.  Flight/dh arcs are
        # still shared at the base level.  The balance here purely ensures that
        # each crew member's home-wait flow is internally consistent — they
        # can't accumulate phantom home rest.

        n_crew_home_bal = 0
        home_nodes_by_base: dict[str, list[Node]] = {}
        for base in self.airports:
            home_nodes_by_base[base] = [
                n for n in node_list if n.airport == base
            ]

        for base in self.airports:
            supply     = self.base_crew[base]
            horizon    = Node(airport=base, time=self.horizon_end)
            home_nodes = home_nodes_by_base[base]
            n_crew_b   = len(crew_ids_by_base[base])
            # Per-crew injection: each crew injects exactly 1 unit at their
            # starting depot.  With position_supply different crew may start
            # at different airports, but all home-wait arcs are at the home
            # airport so every crew's home-wait depot node is (base, 0).
            per_crew_supply_node = Node(airport=base, time=DEPOT_TIME_START)

            for cid in crew_ids_by_base[base]:
                c_out = crew_home_out[cid]
                c_in  = crew_home_in[cid]
                # Only add constraints for nodes where this crew actually has vars.
                active_nodes = set(c_out.keys()) | set(c_in.keys())
                if not active_nodes:
                    continue
                for node in home_nodes:
                    if node not in active_nodes:
                        continue
                    out_expr = gp.quicksum(c_out.get(node, []))
                    in_expr  = gp.quicksum(c_in.get(node, []))
                    if node == per_crew_supply_node:
                        # Depot: each crew injects 1 unit of home-wait flow.
                        rhs = 1
                    elif node == horizon and self.is_last_window:
                        # Final window: crew must also absorb their 1 unit at horizon.
                        rhs = -1
                    else:
                        rhs = 0
                    self.model.addConstr(
                        out_expr - in_expr == rhs,
                        name=f"chbal_{cid}_{node.airport}_{node.time}"
                    )
                    n_crew_home_bal += 1

        self.model.update()
        print(f"  Per-crew home-balance constraints: {n_crew_home_bal:,}  ({_t.time()-t0:.1f}s)")
        ...
        # ── Mandatory home-break constraints (per-crew) ───────────────────────
        #
        # For each crew member c at base b and each MAX_AWAY_DAYS epoch k,
        # crew c must spend at least HOME_BREAK cumulative minutes at home
        # during epoch k.  Because var_wait_home[c, arc] is individualised,
        # this constraint is truly per-crew: one crew cycling home n_b times
        # cannot satisfy it for other crew members.
        #
        # Specifically we require that in each epoch the sum of home-wait flow
        # on qualifying arcs for crew c is >= 1.  A qualifying arc is a home
        # wait arc whose cumulative home-wait time (from the start of the epoch)
        # reaches HOME_BREAK.  Each unit of flow on such an arc represents one
        # crew member spending the required rest there.
        #
        # Cumulative home-wait time is computed the same way as before (walking
        # chronologically from the epoch start, carrying forward accumulated
        # minutes) — but now the constraint RHS is 1 per crew, not n_b in total.

        epoch_min = MAX_AWAY_DAYS * 1440
        n_mand    = 0

        for base in self.airports:
            # For each epoch, find qualifying home-wait arcs (same logic as before).
            epoch_arcs: dict[int, list[Arc]] = defaultdict(list)
            ...
            for arc in sorted(
                ...
                # Only consider arcs for which at least one crew member has a var.
                if not any((cid, arc) in self.var_wait_home
                           for cid in crew_ids_by_base[base]):
                    continue
                ...
                if acc >= HOME_BREAK:
                    ...
                    epoch_arcs[epoch].append(arc)

            for epoch, qualifying_arcs in epoch_arcs.items():
                for cid in crew_ids_by_base[base]:
                    # Collect the per-crew vars on qualifying arcs for this crew.
                    crew_arc_vars = [
                        self.var_wait_home[(cid, arc)]
                        for arc in qualifying_arcs
                        if (cid, arc) in self.var_wait_home
                    ]
                    if not crew_arc_vars:
                        continue
                    # Each individual crew member must pass through at least one
                    # qualifying home-wait arc in this epoch.
                    self.model.addConstr(
                        gp.quicksum(crew_arc_vars) >= 1,
                        name=f"mand_rest_c{cid}_ep{epoch}"
                    )
                    n_mand += 1
    ...
    def _add_coverage_constr(self, f: 'Flight',
        ...
        # Per-crew coverage guarantee: each of the min_crew slots must be filled
        # by a distinct, individually-tracked crew member (var_work_crew ∈ {0,1}).
        # Falls back to the aggregate var_work sum for cached networks built before
        # this change (var_work_crew empty).
        if self.var_work_crew:
            flight_arc_set = set(flight_arcs)
            coverage_expr = gp.quicksum(
                var
                for (cid, arc), var in self.var_work_crew.items()
                if arc in flight_arc_set
            )
        else:
            # Legacy fallback: aggregate base-level coverage (no per-crew guarantee)
            coverage_expr = gp.quicksum(
    ...
    def solve(self, crew_positions: dict[int, str] | None = None) -> dict:
        ...
        # Per-crew flight variables: switch to binary (ub=1 is already set)
        for var in self.var_work_crew.values():
    ...
    def _decompose_routes(
        ...
        arc_is_work_for_crew: set[tuple[int, int]] = set()  # (crew_id, arc_id)
        if self.var_work_crew:
            for (cid, arc), var in self.var_work_crew.items():
                if round(var.X) > 0:
                    arc_is_work_for_crew.add((cid, arc.id))
        else:
            # Fallback for cached networks without var_work_crew: mark all arcs
            # with positive var_work flow as 'work' for any crew of that base.
            ...
            for crew_id, depot in ordered:
                ...
                for arc in path_arcs:
                    if arc.arc_type == 'wait':
                        continue
                    # With per-crew guarantee: check whether this crew member
                    # specifically has a var_work_crew assignment for this arc.
                    # Fallback for cached networks: use base-level arc_is_work_for_base.
                    if arc_is_work_for_crew:
                        leg_type = 'flight' if (crew_id, arc.id) in arc_is_work_for_crew else 'deadhead'
                    else:
...
def solve_rolling_horizon(
    ...
    while win_start < total_min:
        ...
        # from crew_cg import solve_cg
        # result = solve_cg(net, crew_positions=crew_positions)
